...
Here starts the problem: SSAS can only be accessed by an authenticated user on the Server who has Permission.
So as a solution we access SSAS over HTTP and the setup is very straightforward.
1. Create a New Application Pool in IIS and can have .net Framework 2.0 in Classic Mode
2. Change the Application Pool User from "ApplicationPoolIdentity" to a specific User who has access to SSAS
3. Create a New Application in IIS and point it to an empty directory.
